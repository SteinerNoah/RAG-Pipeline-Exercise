...
def create_reranking_model() -> CrossEncoder:
    """Creates a reranking model"""
    return CrossEncoder(CROSS_ENCODER_NAME) 

def rerank_chunks(model: CrossEncoder, question: str, chunks: list[dict[str, object]], final_k: int = FINAL_K) -> list[dict[str, object]]:
    """Ranks the top_k retrieved chunks and selects the final_k to send as context"""
    pairs = [(question, chunk["text"]) for chunk in chunks]
    scores = model.predict(pairs)
    ranked_chunks = []
    for chunk, score in zip(chunks, scores):
        ranked_chunk = chunk.copy()
        ranked_chunk["cross_encoder_score"] = float(score)
...
def main() -> None:
    ...
    similar_chunks = find_similar_chunks(collection, embedding_model, question)
    reranking_model = create_reranking_model()
    reranked_chunks = rerank_chunks(reranking_model, question, similar_chunks)
    ...
    print("\nAntwort:\n") 
    print(answer_question(question, reranked_chunks)) 
